...
ciphersuites). For the \gls{dh} algorithm, the average is computed from $74$ runs ($37$ from each peer). 
...
We will now analyze each one of the algorithms and their costs in more detail.
...
the shared secret. The resulting shared secret will be a 2D coordinate on the curve $(x,y)$. In \gls{tls} the $y$ value is discarded and $x$ is
used as the preshared secret.
