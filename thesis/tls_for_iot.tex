...
In reality, \gls{tls} is composed of several protocols. Before proceeding with a description of each one, \textbf{cipher spec} and \textbf{cipher suite} will be
defined, since they're used throughout the text that follows. The meaning of these terms differs in \gls{tls} $1.2$ and \gls{tls} $1.3$.
...
alongside the definition of the \textbf{key exchange} algorithm and the \gls{prf} (used in key generation).
...
The various protocols that compose \gls{tls} are illustrated in Figure \ref{fig:tls-subprotocols} and briefly described below:
\begin{itemize}
