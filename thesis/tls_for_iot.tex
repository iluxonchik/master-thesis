...
prediction misses. The metrics generated by \textit{Callgrind} can then be loaded into \textit{KCachegrind} 
to visualize the performance results. One of such results is the estimate the number of executed instructions.
\textit{Callgrind} in conjunction with \textit{KCachegrind} are widely used for performance optimization
of executables. In order to estimate the number of CPU instructions executed, we derived a formula from
the one used by \textit{KCachegrind}.
...
In order to estimate the number of executed CPU cycles, \textit{KCachegrind} uses the
...
\textit{Callgrind} only simulates L1 and L2 caches $LLm=0$, therefore the actual formula
used by \textit{KCachegrind} (when used with \textit{Callgrind} output) to estimate the 
number of CPU cycles used is: $CEst = Ir + 10*Bm + 10*L1m + 20*Ge + 100*L2m$.
...
instructions using the \textit{lock} prefix. In our evaluation, we used single-threaded code only, for this 
