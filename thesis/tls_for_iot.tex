...
\begin{itemize}
  ...
  sends it to the server, which in turn decrypts it using the corresponding \gls{privk}. The server then uses it as its premaster secret.
...
Finally, a separation of the "lightweight algorithms" definition into two
...
creates an authorization certificate, protects it with a MAC and sends it
...
The new certificate structure is just the RFC7250 \cite{RFC7250} structure, with an
additional MAC.
...
(defined in \cite{RFC6282}), which is the compressed \gls{dtls} content. The \codeword{LOWPAN_IPHC Encoding}
...
ones observer in \cite{6LoWPANC53:online}), energy consumption (in average \codeword{15\%} less
...
\begin{itemize}
  ...
  key it uses by including a \gls{psk} identity in its \codeword{ClientKeyExchange} message.
  ...
  cipher suite to be used with \gls{psk}s, just like \gls{coap} does. \gls{pfs} is
  a property that preserves the confidentiality of past interactions even if the
  long-term secret is compromised. If a \gls{pfs} cipher suite is used, ephemeral
  ...
  just like with \gls{coap}. Note that the cipher suite makes the use of \gls{aead}.
...
applicable when the \gls{psk}-based cipher suites are used.
...
(D)\gls{tls} cipher suites, may be shorter, dependent on the sensitivity of
transmitted data.
...
The use of symmetric cryptography exclusively, has various additional advantages in the context
...
public key cryptography at all. For some of them, the use of something like raw
