...
show in Section\ref{sec:sec:confid-costs}.
...
We have also used PAPI to measure the number of actual CPU cycles used the time taken. In this section, we will compare the estimates
...
Figure \ref{fig:papi-vs-callgrind-rsa-cost} shows the difference between the costs for \gls{ecdsa} operations reported by \textit{callgrind} 
and \textit{PAPI}.
...
(figure \ref{fig:fig:rsa-ecdsa-pub-priv-ratio}) to the actual values obtained from \textit{PAPI} (\ref{fig:rsa-ecdsa-pub-priv-ratio-papi}).
...
analysis was focused on the handshake and the security services that it offers.
