...
\newacronym{oscp}{OCSP}{Online Certificate Status Protocol}
...
\begin{itemize}
  ...
  ciphersuite to be used with \gls{psk}s, just like \gls{coap} does.
  ...
  \gls{ocsp}\cite{RFC6960}, nor the \gls{clr}\cite{RFC5280} mechanisms are used, instead this task is delegated to
...
\todo{Tell how my work differs from everything described here} In short, right now
I'm thinking of adding something in lines of: the work described above is either
specific to some algorithm or requires introduction of third-party features/intermediaries.
In my solution, I want to take the approach of developing a fully backwards-compatible
solution, while meeting the lighweight requirements of IoT devices. I will explore the
solution that would be general enough to work with both, (D)TLS 1.2 and (D)TLS 1.3.
I will explore TLS specifically, since little work has been done in that area and
everything has been centered around DTLS. I can achieve the previous through
the extension mechanism. There has been no work like RFC 7925 done for TLS 1.3,
I wanna explore that. CoAp over TCP and tls is being proposed, so we need work
done in optimizing tls.
...
\todo{YES, replay attacks. THIS WILL NOT BE A SECTION. It's here so that I will remember
to mention it}
...
It was really designed to be the same as TLS, since even some extensions that
were defined BEFORE dtls exiseted (the psk) can be used in DTLS without any
change.
...
\todo{mention that I contributed to the TLS 1.3 standard}
