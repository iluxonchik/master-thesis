...
\begin{table}[]
  ...
  \end{tabular}
  \caption{Cost of using \textit{PKCS\#1 V2.1 RSAES-PKCS1-v1\_5} encrypiton and decryption schemes with various security levels}
  \end{table}
...
In order to compute the authentcation cost for each peer, we use the values from tables \ref{table:rsa-costs-all-sls} and \ref{table:pkcs-cost}, 
...
Thus, the authentcation cost for the client will be $xx+xx=yy$ and for the server $aa$. Similarly, when an \textit{RSA} or \textit{RSA-PSK} ciphesuite is used at the \textit{normal} security level,
the client will verify the \gls{rsa} signature in the server's certificate and perform a \textit{PKCS\#1 v2.1 RSAES-PKCS1\-V1\_5} encryption, while the server will only need to perform a
\textit{PKCS\#1 v2.1 RSAES-PKCS1\-V1\_5} decryption. Thus, the authentcation cost for the client will be $xx+yy=zz$ and for the server
$lll$. The remaining entries in tables \ref{table:tls-auth-cost-client} and \ref{table:tls-auth-cost-server} are computed in a similar manner.
...
% For cretificate message costs use "mbedtls_ssl_read_record:mbedtls_ssl_parse_certificate"
