I amdoes notthey areI have% This is the file where my Master Thesis will be written. It uses the adapted
% LNCS Template.
%
...
even more devastating, if it is ever put to malicious use and others will inadvertently
come in the future. The \textit{SecurityNow}\cite{GRCSecur72:online} is a weekly podcast
centered around computer security and the topics that surround it. I have been a listener
...
the compromise of privacy, to the \gls{ddos} attacks is a topic that comes up
...
about \gls{tls}, almost everything also applies to \gls{dtls} as well, since it is just
...
depending on the security requirements of the context it is being used in, in that sense,
it would be similar to a framework.

In the process of my work on this master thesis, I have made several
...
solution that I will be developing in the second part of the dissertation, as
well as how I will be evaluating my results, how I am planning to develop my work
and a conclusion.
...
\gls{pubk} crytpography is based on the use of one-way math functions,
...
so I think it is important to distinguish both. \gls{tls} is an evolution of the \gls{ssl} protocol. The protocol changed
...
All 3 security services are not required to be used in every situation.
...
\begin{itemize}
  ...
  the layer that runs directly on top of \textbf{TCP/IP} and it serves as an
   ...
   specified by the \codeword{type} field. This is illustrated in figure \ref{fig:tls-record-header}.
...
Due to limited space, I will not be able to describe \gls{tls} 1.3 in detail. I decided
...
\gls{tls} 2.0 instead. I have studied \gls{tls} 1.3 in great detail, and as I already mentioned,
...
of \gls{iot}. Some of those changes I have already mentioned previously in this
...
so I will begin with the work done in this area.
...
are listed in the paper, alongside relevant metrics. I will not be including the list
...
conventional ones, such as RSA and \gls{ecc}. In my own further research of this
topic, I came to the same conclusion.
...
it was covered in sufficient detail, by purposefully choosing recent works that
...
The document is rather lengthy, only its fundamental parts will be summarized. I will
...
(D)\gls{tls} 1.3, mainly this is because the protocol is still in draft stages, I would not expect
...
specifications or \codeword{RFC 4366} made it clear.

I want to incorporate some of the ideas from \gls{tls} 1.3, such as the
\codeword{0-RTT data}, which can be done if peers already share a \gls{psk}.
