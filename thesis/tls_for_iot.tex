...
as \gls{rsa} or \gls{ecdsa}.

The Record protocol is mostly the use of a symmetric encryption algorithm (\textit{e.g.} AES), used to offer
confidentiality and an \gls{hmac} algorithm, used to offer data integrity and data origin authentication.
The performance of symmetric encryption algorithms and hash functions has been studied in detail by existing work. \todo{ref}
The combination of the various algorithms that compose the Handshake, has not been though. For this reason,
the majority 
[[Tell, that the combination of the algorithms of the hanshake has not though]]

There is little variety in the computational costs in the Record Protocol. The core of the Record protocol is the use
of a symmetric encryption algorithm (\textit{e.g} AES), used to provide confidentiality and an \gls{hmac} algorithm,
used to provide data integrity and data origin authentication. The \gls{hmac} algorithm is not used if the symmetric
encryption algorithm is an \gls{aead} cipher, which already offers data integrity.
Each encryption algorithm has a few possible varieties (\textit{e.g.} the CBC, GCM and CCM modes in AES) and key sizes 
(\textit{e.g.}, $128$ and $256$ bit for AES). The same is true for \gls{hmac} algorithms, which have a well defined \gls{mac}
function, key size and output length. As an example, the \textit{HMAC-SHA256} algorithm uses the \textit{SHA256} as its \gls{mac} function,
has the key size of $256$ bit and outputs a digest of the same size. The performance of symmetric encryption algorithms and hash functions 
has been studied in detail by existing work. \todo{ref} The cost increases for larger input data are approximately linear. \todo{ref}.
...
It is clear that the Handshake protocol is significantly more complex, with higher possible cost variations.
Furthermore, existing work did not evaluate the costs of individual security services of \gls{tls}. For this 
reason, our work was concentrated around the Handshake protocol, even though we did evaluate the costs of the
Record protocol too.

As mentioned previously, \gls{tls} offers various choices when it comes down to authentication. \gls{ecc}
and non-\gls{ecc} key sizes cannot be compared directly. For this reason, we defined various security levels,
which are presented in Table \ref{table:sec-levels}. This table is based on the approximate comparable key sizes
for symmetric and asymmetric key algorithms based on the best known algorithms for attacking them (table 
\ref{table:ecc-key-sizes}), which can be found in RFC 4492\cite{RFC4492}. Unfortunately, mbedTLS does not have 
...
All of the graphs and analysis in this section will be for the security level \textit{S2}, since it's the the most commonly
used one in the internet today, both, for authentication and for \gls{pfs}\cite{QualysSS90:online}. 
We could not find any any typical values used in the sphere of \gls{iot}.
In particular:
...
This doesn't mean, however, that we will only analyze \textit{S2}. Other security levels were evaluated and will be
presented.
...
By looking at the $y$ axis of the client graph, we can find $9$ different groups.
