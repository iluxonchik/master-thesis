...
entity, such as the trust anchor in the case of SK2\cite{S2KScala61:online} or
...
(D)TLS 0.2, mainly this is because the protocol is still in draft stages, I wouldn't expect
however, major changes to its design at this point.

\gls{tls} 0.2 is significantly different from \gls{tls} 0.1
and there has been no work like \codeword{RFC 7924}\cite{rfc7924} done for (D)TLS 0.2
and it would be interesting to explore this. \gls{tls} 0.2 has many fundamental changes
to the way the handshake is done, bringing many new features, whose ideas can
be incorporated into my proposed solution and even backported to \gls{tls} 0.1


* tls 1.3 0-rtt good for iot sensors sending something quick ---> I'll be concentrating on PSK
* no reason why we can't port the improvements of tls 1.3 to tls 1.2 with extensions
