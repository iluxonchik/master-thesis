...
\end{tabular}
\centering
\caption{\label{table:hs-all-ciphers-cli-aggr} Average Handshake cost for the client in millions CPU instructions}
\end{table}
...
\end{tabular}
\centering
\caption{\label{table:hs-all-ciphers-srv-aggr} Average Handshake cost for the server in millions CPU instructions}
\end{table}
...
algorithm for key agreement: $1.354$. In the same manner, the cost of using \textit{RSA} for authentcation can
...
\end{tabular}
\caption{\label{table:pfs-cost-server} \gls{pfs} and \textit{ECDH} key exchange for the server}
\end{table}
...
analyze the total cost of the handshake. 
