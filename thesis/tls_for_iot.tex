...
and evaluated its performance. The results showed that their \gls{ecc} optimizations
...
respect to a standard, non-optimized implementation.

\gls{dtls} was designed to protect web application communication, as a result,
...
guarantee. With this in mind, the authors wanted to design a version of \gls{dlts}
that's both: minimizes the code size and the number of exchanged messages, resulting
in an optimized Handshake protocol.

This work was based on an informational RFC draft\cite{I-D.draft-keoh-dtls-profile-iot-00}, in which the
...
Instead of leaving the fragmentation function to \gls{dtls}, the authors
delegated it to the block-wise transfer feature of \gls{¢oap}\cite{RFC7959}, which was developed
...
many security protocols rely upon.
