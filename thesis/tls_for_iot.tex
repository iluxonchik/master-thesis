...
of a big part of \gls{iot} devices does not permit the use of computationally complex
...
can not be easily integrated with existing deployments. This paper describes
how the \gls{tls} extension mechanism can be used to define a framework
that can adapt the protocol to specific needs. This is the approach that
...
\gls{tls} will be mentioned, almost everything also applies to \gls{dtls}.  This is a consequence of \gls{dtls} being just an adaption of \gls{tls} over unreliable transport protocols, with no changes done
the core protocol.
...
a large amount of small information constantly. Let us consider the case of a temperature sensor
...
depending on the security requirements of the context of it is used in.

In the process of the work on this dissertation, we have made several
contributions to the \gls{tls} $1.3$ specification and recognized as contributors\cite{Mergepul65:online}.
...
an explanation of how the results will be evaluated and a general work plan. Finally, the conclusion of the work is done in Section 6.

\section{Background}

\gls{tls} is a complex protocol that relies on various concepts to provide
security. Some of them will be described here.

\gls{tls} uses \gls{ac} for peer authentication and \gls{sc} for bulk data
encryption, for this reason this topic will be covered in Section \ref{sac}. Section \ref{pccc} covers the most common way of peer authentication: \gls{pubk} certificates. \gls{aead} ciphers offer various advantages in the
...
When compared to other \gls{pubk} cryptography approaches, \gls{ecc} offers shorter keys, lower central processing unit (CPU) consumption and lower memory usage for equivalent security strength. It is also heavily used
in \gls{tls}. For those reasons, this section ends with an overview of \gls{ecc} in Section \ref{eccsection}.


\subsection{Symmetric vs Asymmetric Cryptography} \label{sac}

\gls{ac} is more expensive than \gls{sc} in terms of performance. This has two main reasons. First, larger key sizes are required for an \gls{ac} system to achieve the
...
(located in the \codeword{rsa.c} file). The conclusion is that that using the \codeword{rsa.c} module adds an overhead of \codeword{32KB}.
...
document used to prove the ownership of a \gls{pubk}. This allows other parties
...
\gls{pubk} cryptography is based on the use of one-way math functions. Such
...
The connection begins with the client sending a \codeword{ClientHello}, containing \codeword{random}, \codeword{cipher_suites} and \codeword{compressison_methods},
among other fields.
...
a list of supported features, it contains a \textbf{single item} containing the one that it chose. The server responds with a single \codeword{cipher_suite} and
...
means of a full handshake. In its \codeword{ServerHello}, the server generates and sends a \codeword{session_id}, which will be associated with the newly negotiated session.

To resume a session, in its \codeword{ClientHello} the client includes a \codeword{session_id} of the session it wants to
...
will be able to impersonate the compromised party until the corresponding \codeword{session ID} is retired.
...
This processing is done by the \textbf {TLS Record Protocol} and involves the following steps (\codeword{4} for \gls{tls} $1.2$ and \codeword{3} for \gls{tls} $1.3$):
...
needed for a connection. It is defined as \codeword{PRF(secret, label, seed) = P_hash(secret, label + seed)}.
...
\begin{itemize}
  ...
  server's \gls{pubk} (which it obtained from the server's \codeword{X.509} certificate) and
...
Lightweight cryptography an important topic in the context of \gls{iot} security, due to the resource-limited nature of the devices. This section will begin with the description of the work done in this area.
...
While there is a high demand for lightweight \gls{pubk} primitives, the required
...
From early March $2018$ until April $2018$ the solution will be evaluated and a concentration on writing the dissertation's text will be done.
