...
of CPU cycles executed. To be more precise, an estimate of that value. In the next section we will analyse the
...
still make sense for IoT, because it might be just "good enough" encryption.
...
number of CPU  cycles, in millions, used to perfrom the handshake for that key exchange method. The values are taken
...
are taken directy from table \ref{table:hs-all-ciphers-cli-aggr} and will be in millions CPU cycles, 
rounded to $3$ decimal places.

If \gls{pfs} is used, the cost of the Handshake varies from $55.805$ to $135.443$ 
million CPU instrutions for \gls{psk} based authentcation, and from $58.608$ to $168.954$ million CPU cycles for 
...
is of $114.905 (4.559+110.346)$ million CPU cycles. When compared, authentcation with \gls{rsa} is
...
million CPU instrutions for \gls{psk} based authentcation, and from $86.097$ to $212.255$ million CPU cycles for 
asymmetric authentcation. If the seucirity service of \gls{pfs} is foregone, the cost
...
authentication and \textit{ECDSA} authentcation costs $46.336$ ($132.433-86.097$) million CPU cycles less 
...
is of $30.925 (77.261-46.336)$ million CPU cycles. When compared, authentcation with \gls{ecdsa} is
...
In the prevoius seciton we approximated the cost of \gls{psk} authentcation to be $1.119$ million CPU cycles 
...
This value can be seen as the "TLS Handshake" ovehead. Approximately $69\%$ of those CPU cycles are spent on
...
cycles are spent on reading/wrting to/from the network and computing the remaining \gls{tls} Handshake messages.
...
\subsubsection{The Cost Of Authenticaition in TLS} \label{sec:auth-cost-in-tls}
...
keys or $256$ bit \gls{ecdsa} keys. Thus, for the certificate signature verficiation cost on the client side, the values from the \textit{normal} 
...
\subsection{Perfect Forward Secrecy Cost Analysis}
...
of CPU cycles used when performing the Hanshake with the specified key exchage method and security level. 
...
In section \ref{sec:auth-cost-in-tls} we analysed the cost of authentcation in \gls{tls} and in section \ref{sec:pfs-cost-in-tls}
the cost of \gls{pfs} in \gls{tls}. Tables \ref{table:tls-auth-cost-client}, \ref{table:pfs-cost-server}, 
\ref{table:pfs-cost-client} and \ref{table:pfs-cost-server} show the authentcation and \gls{pfs} costs for the client and the server, 
...
is not enough to sum the corresponding values from the authentcation and \gls{pfs} cost tables. The overhead of establishing the \gls{tls} connection and 
...
This message is omitted entirely in the \textit{PSK} key exchage. In ciphersuites that use asymmetric authentcation, 
...
Thus, we can use the following formula to compute the cost of the \gls{tls} Handshake cost for a key exchage method and
security level: $Handshake Cost = TLS Overhead + Auth Cost + PFS Cost + Additional Costs$. The \textit{TLS Overhead} is the cost of the
\textit{PSK} key exchage and can be obtained from table \ref{table:client-hs-cost-all-sls} for the client
and from table \ref{table:server-hs-cost-all-sls} for the server. \textit{Auth Cost} is the cost of authentcation and can be obtained
...
and security level. This costs do not include checking the ceritifcate's signature, but rather reading the certificate from the record layer, 
parsing the \textit{der}-ecoded certificate into internal fields, checking the validity of each field and creating a hash of the fields. 
...
key exchange, from the \gls{tls} security service costs that we presented in secitons \ref{sec:auth-cost-in-tls} and \ref{sec:pfs-cost-in-tls} 
...
For the additional certificate parising costs, we will consult table \ref{table:cert-parse-cost}. Since the certificate is an \gls{rsa} signed one,
we wil use the value from \textit{RSA} row and \textit{high} column: $607542$. All that's left now is insert those values in the formula:
